...
module.exports = function kindOf(val) {
  ...
  // primitivies
  if (vtypeof === 'undefined') return 'undefined'
  if (val === null) return 'null'

  if (val === true || val === false || val instanceof Boolean) return 'boolean'
  if (vtypeof === 'string' || val instanceof String) return 'string'
  if (vtypeof === 'number' || val instanceof Number) return 'number'
  // functions
  if (vtypeof === 'function' || val instanceof Function) return 'function'
  // array
  ...
  if (type === '[object Promise]') return 'promise'
